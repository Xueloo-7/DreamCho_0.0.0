    ...
    public class Key : Singleton<Key>
    {
        ...
        private void Update()
        {
            switch (GameManager.appMode)
            {
                case AppMode.Android:
                    ...
                    //GetAxis(InputDir.Horizontal); GetAxis(InputDir.Vertical); // debug
                    //GetAxisRaw(InputDir.Horizontal); GetAxisRaw(InputDir.Vertical); // debug
                    break;
                case AppMode.PC:
            ...
            /*if (GetButtonDown(K.Power)) KeyEvent.onPowerDown?.Invoke();
            if (GetButtonUp(K.Power)) KeyEvent.onPowerUp?.Invoke();
            if (GetButton(K.Power)) KeyEvent.onPower?.Invoke();

            if (GetButtonDown(K.Character)) KeyEvent.onCharSwitchDown?.Invoke();
            if (GetButtonUp(K.Character)) KeyEvent.onCharSwitchUp?.Invoke();
            if (GetButton(K.Character)) KeyEvent.onCharSwitch?.Invoke();

            if (GetButtonDown(GameKey.Attack) || CheckRemainInput("Attack_Down")) KeyEvent.onAttackDown?.Invoke();
            if (GetButton(GameKey.Attack) || CheckRemainInput("Attack")) KeyEvent.onAttack?.Invoke();
            if (GetButtonUp(GameKey.Attack) || CheckRemainInput("Attack_Up")) KeyEvent.onAttackUp?.Invoke();

            if (Input.GetMouseButtonDown(0)) KeyEvent.onMouseDown?.Invoke();
            if (Input.GetMouseButton(0)) KeyEvent.onMouse?.Invoke();
            if (Input.GetMouseButtonUp(0)) KeyEvent.onMouseUp?.Invoke();*/
        ...
        private IEnumerable<KeyCode> KeyToKeys(GameKey gameKey)
        {
            switch (gameKey)
            {
                ...
                case GameKey.Character: return K.Character;
